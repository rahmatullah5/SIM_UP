<?php
						...
						if(	$tipe != "image/jpg" && $tipe != "image/jpeg" && $tipe != "image/pjpeg" && $tipe != "image/png" && $tipe != "image/gif" && $_FILES["upload"]["size"] < 1000000)
						{
							echo "Error";					
								...
								if (move_uploaded_file($_FILES['gbr']['tmp_name'], $folder)) 
								{
									...
									$q=mysql_query("insert into su_produk (nama,deskripsi,id_kategori,status_produk,harga,diskon,stok,gambar) values ('$a', '$b' , $c , $d , $e , $f , $g , '$gl')") or die(mysql_error());
	...
	switch ($_GET['b']) {
		...
		$c=anti_inject($_POST['id']);
			...
			$c=anti_inject($_POST['nama']);
			
			$d=anti_inject($_POST['tentang']);
			$id=anti_inject($_POST['id']);

			$gambar = anti_inject(($_FILES['gbr']['name']));
					if(!is_dir("../../assets/images/admin/". $id ."/")) {
					    mkdir("../../assets/images/admin/". $id ."/");
					}
					$folder = "../../assets/images/admin/". $id ."/";
					$folder = $folder.basename($_FILES['gbr']['name']);
					$tipe =$_FILES['gbr']['type'];
					//$extension = end(explode(".", $_FILES["file"]["name"]));
						if(	$tipe != "image/jpg" && $tipe != "image/jpeg" && $tipe != "image/pjpeg" && $tipe != "image/png" && $tipe != "image/gif" && $_FILES["upload"]["size"] < 1000000)
						{
							echo "Error";					
								...
								if (move_uploaded_file($_FILES['gbr']['tmp_name'], $folder)) 
								{
									$gl="assets/images/admin/". $id ."/" . $gambar;
									...
									$q=mysql_query("update su_admin set user='$a' , pass='$b' , nama= '$c' , tentang= '$d' ,gambar='$gl' where id_admin=$id") or die(mysql_error());
