...
<body>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					$periksa=mysql_query("select * from su_user where user_level<=0");
					if(mysql_num_rows($periksa)!=0){
						while($q=mysql_fetch_array($periksa)){	
						if($q['user_level']<=0 or empty($q['user_level'])){			
							echo "<div style='padding:5px' class='alert alert-warning'><span class='glyphicon glyphicon-info-sign'></span> User :  <a href='det.php?a=user&id=$q[id_user] 'style='color:red'>". $q['nama_d']." ".$q['nama_b']." </a> belum diaktifkan !!</div>";	
							$tt+=1;
							echo '<script>
					$(document).ready(function(){
						$("#pesan_sedia").css("color","red");
	...
	<div class="col-md-2 sidebar-admin">
		<div class="row">
			<?php 
			$use=$_SESSION['uname'];
			$fo=mysql_query("select gambar from su_admin where user='$use'");
		...
		<ul>
		   ...
		   <li class='has-sub'><a href='#'><span style="float:left" class="glyphicon glyphicon-briefcase"></span><span>Barang</span></a>
		      <ul>
		         ...
		         <li><a href="barang_laku.php"><span>Olah Barang Laku</span></a></li>
		         <li><a href='kategori.php?a=a'><span>Olah Kategori</span></a></li>
		         <li class='last'><a href='status_barang.php'><span>Olah Status</span></a></li>
		      </ul>
		   </li>
		   <li class='has-sub'><a href='#'><span style="float:left" class="glyphicon glyphicon-user"></span><span>User</span></a>
		   	<ul>
		         <li><a href='user.php'><span>Olah User</span></a></li>
		         <!--<li class='last'><a href='#'><span>Olah Level</span></a></li>!-->
		      </ul>
		   </li>
		   ...
		   <ul>
		         ...
		         $fo=mysql_query("select * from su_admin where user='$use'");
					$f=mysql_fetch_array($fo);
		         echo $f['id_admin']; ?>"><span>Olah Profile</span></a></li>
		    </ul>
		    </li>
