...
<table class="table table-hover">
	<tr>
		...
		<th class="col-md-4">ID Cart</th>
		<th class="col-md-3">Nama Pembeli</th>
		<th class="col-md-1">Jumlah Pembelian</th>
	...
	if(isset($_GET['cari'])){
		...
		$brg=mysql_query("select * from su_pesanan where nama like '%$cari%' ");
	}else{
		$brg=mysql_query("select * from su_pesanan group by tgl limit $start, $per_hal ");
	}
	$no=1;
	while($b=mysql_fetch_array($brg)){
		...
		<tr>
			...
			<td><?php echo "$b[id_cart]"  ?></td>
			<td><?php echo "$b[nama_d] $b[nama_b]"  ?></td>
			
			<td><?php $total_p=0;$c=mysql_query("select * from su_cart where id_cart='$b[id_cart]'"); while ($d=mysql_fetch_array($c)) {
				 $e=mysql_query("select * from su_produk where id_produk=$d[id_produk]");$f=mysql_fetch_array($e);
				 if ($f['diskon']>0) {
				 	
				 	$d['harga']=$d['harga']-(($d['harga']/100)*$f['diskon']);

				 }
				 $total_p+=$d['harga']*$d['banyak_produk'];
			} echo "Rp.".number_format($total_p); ?></td>
			<td>
				<a href="det.php?a=pesanan&id=<?php echo $b['id_pesanan']; ?>" class="btn btn-info">Detail</a>
				
				<a onclick="if(confirm('Apakah anda yakin ingin menghapus data ini ??')){ location.href='_func/query.php?c=pesanan&id=<?php echo $b['id_pesanan']; ?>' }" class="btn btn-danger">Hapus</a>
			</td>
		</tr>		
		<?php 
	}
	...
	<tr>
		...
		<?php 
			$xxx=0;
			$a=mysql_query("select * from su_pesanan group by tgl");
			$total_p=0;
			while ($b=mysql_fetch_array($a)) {
			 $c=mysql_query("select * from su_cart where id_cart='$b[id_cart]'"); while ($d=mysql_fetch_array($c)) {
				 $e=mysql_query("select * from su_produk where id_produk=$d[id_produk]");while ( $f=mysql_fetch_array($e)) {
				  if ($f['diskon']>=0) {
				 	$d['harga']=$d['harga']-(($d['harga']/100)*$f['diskon']);
				 }
				 $total_p+=$d['harga']*$d['banyak_produk'];
			...
			echo "<b> Rp.". number_format($total_p).",-</b>";		
		?>
		</td>
	</tr>
</table>
