...
<table class="table">
	...
	if(isset($_GET['tanggal'])){
		...
		$brg=mysql_query("select * from su_cart_ref");
	}
	$no=1;
	while($b=mysql_fetch_array($brg)){
		$a=mysql_query("select * from su_produk where id_produk=$b[id_produk] and  id_user=$_SESSION[id_login]");
		$a1=mysql_fetch_array($a);
		?>
		<tr>
			<td><?php echo $no++ ?></td>
			
			<td colspan="3"	><?php echo $a1['nama'] ?></td>
			<td>Rp.<?php echo number_format($a1['harga']) ?>,-</td>
			<td>Rp.<?php echo number_format($a1['harga']*$b['banyak_produk']); $total+=$a1['harga']*$b['banyak_produk']; ?>,-</td>
			<td><?php echo $b['banyak_produk'] ?></td>			
			
			<td>		
				<!--<a href="edit_laku.php?id=<?php echo $b['id']; ?>" class="btn btn-warning">Edit</a> !-->
				<a onclick="if(confirm('Apakah anda yakin ingin menghapus data ini ??')){location.href='_func/query.php?c=produk_laku&id=<?php echo $b['id_produk']; ?>' }" class="btn btn-danger">Hapus</a>
			</td>
		</tr>
	...
	<tr>
		...
		if(isset($_GET['tanggal'])){
			...
			//$xx=mysql_fetch_array($x);			
			echo "<td><b> Rp.". number_format($total).",-</b></td>";
		}
